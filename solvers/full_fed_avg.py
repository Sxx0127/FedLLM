...
from utils import get_num_classes, get_dataloader, get_multi_dataloader, initialize_networks
...
class FullFedAvgSolver(object):
    ...
    def __init__(self, args, client2data: dict, cls_dist=None):
        ...
        self.global_net = initialize_networks(dataset=args.dataset, model=args.model, adapter=args.adapter,
                                              rank=args.rank, con_dim=args.con_dim)
        ...
        for client_id in self.client2data:
            ...
            self.data_indices[client_id] = dataidx
            loader, _ = get_dataloader(dataset=self.args.dataset,
    ...
    def load_model(self, round):
        file_name = '{}_{}_{}_{}_{}_{}_{}.pt'.format(self.args.model, self.args.alpha, self.args.n_parties,
                                                     self.args.alg, self.args.adapter, self.args.sample_fraction, round)
        self.global_net = pickle.load(open(os.path.join(self.args.modeldir, file_name), 'rb'))

    def save_model(self, round):
        # file_name = '{}_{}_{}_{}_{}_{}_{}.pt'.format(self.args.model, self.args.alpha, self.args.n_parties,
        #                                           self.args.alg, self.args.adapter, self.args.sample_fraction, round)
        file_name2 = '{}_{}_{}_{}_{}_{}_{}_{}.safetensors'.format(self.args.model, self.args.alpha, self.args.n_parties,
                                                                  self.args.alg, self.args.adapter,
                                                                  self.args.sample_fraction, round, self.args.epochs)
        # pickle.dump(self.global_net, open(os.path.join(self.args.modeldir, file_name), 'wb'))
        save_file(self.global_net.state_dict(), os.path.join(self.args.modeldir, file_name2))
